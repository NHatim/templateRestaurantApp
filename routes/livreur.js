...
router.post(
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      res.render('espace-livreur', {
        title: 'Eat It | Section Livreur liste restaurant',
        nom: req.session.nom,
        prenom: req.session.prenom,
        tabRestaurant: commandeApi.loadRestaurants,
      });
    }
  }
);
...
router.post('/choix-restaurant', (req, res, next) => {
  let findResto = commandeApi.loadRestaurants.find(
    (element) => req.body.restaurant.trim() === element.nom
  );
  let tabNumCom = [];

  commandeApi.tabCommande.forEach((element) => {
    if (req.body.restaurant.trim() === element.restaurant.nom) {
      if (
        element.etatCommande === 'pret' ||
        element.etatCommande === 'enlever'
      ) {
        tabNumCom.push(element.numero);
      }
    }
  });
  res.render('livreur-restaurant', {
    title: 'Eat It | Section Livreur confirmation restaurant',
    nom: req.session.nom,
    prenom: req.session.prenom,
    resto: findResto,
    tabCommande: commandeApi.tabCommande,
    tabNumCom: tabNumCom,
  });
});

router.post('/confirmation-commande', (req, res, next) => {
  commandeClient = commandeApi.tabCommande.find(
    (element) => element.numero === parseInt(req.body.commande)
  );
  commandeClient.heureEnlevement = new Date().toString().slice(16, 24);
  if(commandeClient.etatCommande === 'pret'){
    commandeClient.restaurant.commandeEnlever++;
    commandeClient.etatCommande = 'enlever';
  }
  res.render('livreur-confirmation', {
    title: 'Eat It | Section Livreur confirmation commande',
